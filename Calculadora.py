...
def show_buttons():
    ...
    btnBack.place(x=185, y=42)
    btnModulo.place(x=10, y=72)
    btnMul.place(x=64, y=72)
    btnDiv.place(x=125, y=72)
    btnSub.place(x=185, y=72)
    btnMEM.place(x=242,y=72)
    btnSoma.place(x=185, y=102)
    btnSete.place(x=10, y=102)
    btnOito.place(x=64, y=102)
    btnNove.place(x=125, y=102)
    btnQuatro.place(x=10, y=132)
    btnCinco.place(x=64, y=132)
    btnSeis.place(x=125, y=132)
    btnUM.place(x=10, y=162)
    btnDois.place(x=64, y=162)
    btnTres.place(x=125, y=162)
    btnZero.place(x=10, y=192)
    btnDecimal.place(x=64, y=192)
    btnIgual.place(x=125, y=192)        

def memoria():
    """Função para manipular a memória."""
    global expression
    global memoria_value
    
    # Verifica se há algo no campo de entrada
    if expression:
        # Se houver algo, armazena a expressão junto com o valor anterior da memória
        if memoria_value:
            memoria_value += expression
        else:
            memoria_value = expression
        # Atualiza o campo de entrada para exibir a mensagem de sucesso
        EEcra.delete(0, customtkinter.END)
        EEcra.insert(0, "Valor armazenado na memória.")
    else:
        # Se o campo de entrada estiver vazio, informa ao usuário
        EEcra.delete(0, customtkinter.END)
        EEcra.insert(0, "Nada para armazenar na memória.")
...
btnBack = customtkinter.CTkButton(janela, text='←', width=100,font=("Arial", 12, 'bold'),bg_color='white')
btnBack.place(x=185,y=42)

BtnON = customtkinter.CTkButton(janela, text='ON', width=55, font=("Arial", 12, 'bold'),bg_color='white')
BtnON.place(x=64,y=42)

btnOFF = customtkinter.CTkButton(janela, text='OFF', width=55,font=("Arial", 12, 'bold'),bg_color='white')
btnOFF.place(x=125,y=42)

btnModulo = customtkinter.CTkButton(janela, text='%', width=50,font=("Arial", 12, 'bold'),bg_color='white')
btnModulo.place(x=10,y=72)

btnMul = customtkinter.CTkButton(janela, text='X', width=55,font=("Arial", 12, 'bold'),bg_color='white')
btnMul.place(x=64,y=72)

btnDiv = customtkinter.CTkButton(janela, text='/', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnDiv.place(x=125,y=72)

btnSub = customtkinter.CTkButton(janela, text='-', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnSub.place(x=185,y=72)

btnMEM = customtkinter.CTkButton(janela, text='MEM', width=40, height=149, font=("Arial", 12, 'bold'),bg_color='white')
btnMEM.place(x=242,y=72)

btnSete = customtkinter.CTkButton(janela, text='7', width=50,font=("Arial", 12, 'bold'),bg_color='white')
btnSete.place(x=10,y=102)

btnOito = customtkinter.CTkButton(janela, text='8', width=55,font=("Arial", 12, 'bold'),bg_color='white')
btnOito.place(x=64,y=102)

btnNove = customtkinter.CTkButton(janela, text='9', width=55,font=("Arial", 12, 'bold'),bg_color='white')
btnNove.place(x=125,y=102)
...
btnCinco = customtkinter.CTkButton(janela, text='5', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnCinco.place(x=64,y=132)

btnSeis = customtkinter.CTkButton(janela, text='6', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnSeis.place(x=125,y=132)

btnUM = customtkinter.CTkButton(janela, text='1', width=50, font=("Arial", 12, 'bold'),bg_color='white')
btnUM.place(x=10,y=162)

btnDois = customtkinter.CTkButton(janela, text='2', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnDois.place(x=64,y=162)

btnTres = customtkinter.CTkButton(janela, text='3', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnTres.place(x=125,y=162)

btnZero = customtkinter.CTkButton(janela, text='0', width=50, font=("Arial", 12, 'bold'),bg_color='white')
btnZero.place(x=10,y=192)

btnDecimal = customtkinter.CTkButton(janela, text=',', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnDecimal.place(x=64,y=192)

btnIgual = customtkinter.CTkButton(janela, text='=', width=55, font=("Arial", 12, 'bold'),bg_color='white')
btnIgual.place(x=125,y=192)
#----------------------------------------------------------------------------------------------
# criar a Label Autor -------------------------------------------------------------------------
...
btnMEM.configure(command=lambda: memoria())
